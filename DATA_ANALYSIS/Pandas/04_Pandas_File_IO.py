...
'''
    CSV는 가장 널리 사용되는 데이터 파일 형식
    데이터 분석, 데이터베이스 내보내기/가져오기, 시스템 간 데이터 교환에 주로 사용됨
...
'''
CSV는 기본적으로 쉼표(,)로 구분하지만, 다른 구분자도 사용 가능
예: 탭(\t), 세미콜론(;), 파이프(|) 등
'''
...
# Excel 파일 다루기
'''
    Excel은 마이크로소프트의 스프레드시트 프로그램
    CSV보다 더 많은 기능을 제공하지만, 파일 크기가 크고 처리 속도가 느릴 수 있음
...
'''
    JSON은 웹 API, 설정 파일, 데이터 교환에 널리 사용되는 텍스트 기반 형식
...
# 3. iloc - 정수 위치(integer location) 기반 인덱싱
'''
