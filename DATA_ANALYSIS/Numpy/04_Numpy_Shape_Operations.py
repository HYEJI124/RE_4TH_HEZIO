...
# 3. 스칼라와의 연산 (Broadcasting의 기본 형태) - 스칼라(단일 값)는 자동으로 배열의 모든 요소와 연산됨
...
# 4. 2차원 배열(행렬) 간의 연산 - 같은 크기의 행렬끼리는 요소별로 연산됨
...
# 행렬 곱셈: A @ B -> 결과[i,j] = A의 i번째 행과 B의 j번째 열의 내적
print('행렬 곱셈 (A @ B):\n', A @ B)
'''
계산 과정:
...
# 10. 축을 따른 누적 연산 - cumsum과 cumprod도 axis를 지정할 수 있음
