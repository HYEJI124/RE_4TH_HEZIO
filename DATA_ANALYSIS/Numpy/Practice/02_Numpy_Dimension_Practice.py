...
ravel과 flatten을 각각 사용해 1차원 배열로 변환하고, arr의 첫 번째 원소(arr[0, 0])를 999로 바꾼 뒤 
ravel 결과와 flatten 결과에 어떤 변화가 있는지 확인
'''
arr = np.array([[10, 20], [30, 40], [50, 60]])
...
'''
2. 크기가 32×32인 이미지 데이터를 가정하고, 이 배열에 대해 expand_dims를 사용하여 shape(1, 32, 32)로 바꾸는 코드를 작성하세요.
...
'''
4. 아래 2차원 배열을 
1) 1차원 배열로 만든 후, 2) 중복값을 제거한 뒤 shape (1, n)으로 재구성하세요.
