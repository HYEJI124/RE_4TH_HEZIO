...
# 인덱싱을 통한 값 수정 - 특정 인덱스의 값을 직접 변경할 수 있음
arr[0] = 100
...
# 5. 슬라이싱으로 값 수정 - 슬라이싱으로 선택한 범위의 값들을 한 번에 수정 가능

# 1) 단일 값으로 수정 - 선택한 모든 요소가 같은 값으로 변경됨
arr[2:5] = 100
print('인덱스 2~4를 100으로 수정:\n', arr)

# 2) 배열로 수정 - 선택한 요소들을 각각 다른 값으로 변경
...
# 11. 팬시 인덱싱으로 특정 행들 선택 - 정수 배열을 사용하여 원하는 행들만 선택
row_indices = [0, 2, 3]
print('[0, 2, 3]번 행 선택:\n', matrix[row_indices])
# 결과: [[ 1  2  3  4  5]
#       [11 12 13 14 15]
...
# 14. 불린 인덱싱 - 2차원 배열에서도 조건을 만족하는 요소들을 선택 가능
...
# 15. 불린 인덱싱으로 값 수정 - 조건을 만족하는 모든 요소를 한 번에 수정 가능
