...
'''
딕셔너리의 5가지 주요 특징:
1. Key-Value 쌍: 각 값에 고유한 키로 접근
2. 빠른 검색: O(1) 시간 복잡도로 데이터 접근
3. 변경 가능(Mutable): 요소 추가, 수정, 삭제 가능
4. Key는 유일: 중복 키 불가 (값은 중복 가능)
5. 순서 보장: Python 3.7+ 부터 삽입 순서 유지
'''
...
'''
Unhashable(해시 불가능한) 타입은 key로 사용 불가 - 가변 타입
...
성능 비교:
- 검색: 딕셔너리 O(1) vs 리스트 O(n)
- 삽입: 딕셔너리 O(1) vs 리스트 O(1) (끝에 추가)
- 삭제: 딕셔너리 O(1) vs 리스트 O(n) (중간 삭제)

딕셔너리 사용 시 주의사항

1. 키는 불변 타입만 사용 가능
2. 키의 순서는 Python 3.7+ 에서만 보장
3. 중첩 딕셔너리 복사 시 깊은 복사 고려
4. 키 존재 여부 확인 후 접근하는 습관
5. 메모리 사용량이 리스트보다 많음
'''