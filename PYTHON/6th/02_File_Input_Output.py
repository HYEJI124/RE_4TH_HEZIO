...
'''
[위험한 방법 - 수동으로 파일 닫기]
문제점: 예외 발생 시 파일이 제대로 닫히지 않을 수 있음
'''
...
주의: 기존 파일이 있으면 내용이 완전히 삭제되고 새로 씀
'''
...
장점: 간단함 | 단점: 대용량 파일은 메모리 부족 위험
'''

with open('test_data.txt', 'r', encoding='utf-8') as f:
    print(f'파일 포인터 처음 위치: {f.tell()}')  # tell(): 현재 파일 포인터 위치
    content = f.read()  # 전체 내용을 한 번에 읽음 (메모리 사용량 많음)
    print("전체 내용:")
    print(content)
...
반환값: ['첫줄\\n', '둘째줄\\n', '셋째줄\\n', ...] 형태의 리스트
주의: 전체 파일을 메모리에 로드하므로 대용량 파일에는 부적합
'''
...
# 바이너리 파일 처리 (이미지, 동영상 등)

'''
[ 바이너리 파일 복사 예제 ]
'rb': 바이너리 읽기 모드, 'wb': 바이너리 쓰기 모드
'''
