...
'''
시뮬레이션: 사용자가 'abc' → 25를 순서대로 입력했다고 가정

숫자로만 입력해주세요! -> 다시 시도해보세요. -> 입력된 나이: 25세
'''
...
'''
완전한 예외 처리 구조: try-except-else-finally
'''
...
def complete_exception_structure():
    ...
    for case_name, test_func in test_cases:
        ...
        try:
            ...
            print("2. except 블록 실행")
            print(f"예외 처리: {e}")

        except Exception as e:
            print("3. except 블록 실행 (일반 예외)")
            print(f"예상치 못한 예외: {e}")
...
주의사항:
- except문만 사용하면 모든 예외를 숨길 수 있어 위험
