...
def saved_phone(input_id):
    ...
    try:
        ...
        with open('member_tel.txt', 'w', encoding='utf-8') as f2:
            # 딕셔너리의 모든 항목을 파일에 저장
            for name, phone in members.item():
                # 각 줄에 '이름 전화번호' 형태로 저장
        ...
        # 최종 저장 상태 확인
        print('최종 저장된 전화번호 목록: ')
        for name, phone in members.items():
            status = 'new' if name == input_id and not old_phone else 'change' if name == input_id else ''
        ...
        with open('member.txt', 'r', encoding='utf-8') as f:
            ...
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    print(f'{line_num}번째 줄 형식 오류: "{line.strip()}"')
                    continue
