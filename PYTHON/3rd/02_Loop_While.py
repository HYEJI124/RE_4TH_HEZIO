...
while count > 0:  # count가 0보다 크면 계속 반복
    print(f'count {count}')
    count = count - 1  # 매번 1씩 빼기 (5→4→3→2→1→0)
    # count = count - 1이 없으면 5에서 영원히 멈춰있음 (무한반복)

print('while문 종료')  # count가 0이 되면 실행됨
...
# ==========================================
print("\n=== 나이 입력 받기 ===")
# 사람 나이는 0~150살 사이로 지정
age = -1  # 일부러 잘못된 값 넣기 (반복문 들어가려고)

while age < 0 or age > 150:  # 나이가 이상하면 다시 물어보기
    age = int(input('나이를 입력하세요(0-150): '))

    if age < 0 or age > 150:  # 다시 이상한 값 입력되는 경우
        print('부적절한 값입니다. 다시 입력해주세요!')
...
# ==========================================
print("\n=== 비밀번호 입력 (3번 기회) ===")
correct_password = 'python123'  # 비밀번호
attempt = 0                      # 지금까지 시도 횟수
max_attempts = 3                 # 총 3번 기회

while attempt < max_attempts:  # 3번 될 때까지 반복
    password = input('비밀번호를 입력하세요: ')
    attempt = attempt + 1  # 시도 횟수 1 증가

    if password == correct_password:  # 올바른 비밀번호
        print('로그인 성공!')
        break  # 반복문 탈출!
    else:  # 비밀번호 틀린 경우
        remaining = max_attempts - attempt  # 남은 횟수
...
# ==========================================
print("\n=== while-else 예제 ===")
# while-else는 break 없이 정상 종료됐는지 확인할 때 사용
...
2. 조건을 변경하는 코드 꼭 넣기 (안 그러면 무한반복)
3. break: 반복문 즉시 탈출
4. continue: 이번 반복만 건너뛰기
5. while True: 무한반복 (break로 탈출)      
'''
