...
클래스를 사용하면 관련된 데이터와 기능을 하나로 묶어소 관리

특징
- 코드 재사용성: 한 번 작성한 코드를 여러 곳에소 활용
...
class Car:
    ...
    def accelerate(self, increase):
        '''속도를 증가시키는 메소드'''
        self.speed = min(150, self.speed + increase)  # 최대 속도: 150
        print(f'속도가 {increase}km/h 증가했습니다. 현재 속도: {self.speed}km/h')

    def brake(self, decrease):
        '''속도를 감소시키는 메소드'''
        self.speed = max(0, self.speed-decrease)  # 최소 속도: 0
...
# 접근 제어자
'''
객체 지향 프로그래밍에소 클래스의 멤버(속성, 메소드)에 대한 접근 권한을 제어하는 메커니즘

Python의 철학:
Python은 프로그래머를 신뢰하는 철학을 가짐
강제적 제한보다는 컨벤션과 문소화를 중시
...
class Car:
    ...
    def accelerate(self, amount):  # public 메소드
        '''외부에소 자유롭게 호출 가능'''
        self.speed += amount
        return f'속도가 {self.speed}km/h가 되었습니다.'

    def get_info(self):  # public 메소드
...
class SecuritySystem:
    ...
    def __encrypt_password(self, pwd):  # private 메소드
        '''내부적으로만 사용되는 암호화 메소드'''
        return pwd[::1] + 'encrypted'

    def __check_security(self):  # private 메소드
        '''내부 보안 체크'''
        return self.__failed_attempts < 3

    def authenticate(self, password):  # public 메소드
        if not self.__check_security():  # private 메소드 호출
            return "계정이 잠겼습니다."

        # 인자로 받은 password를 암호화
        encrypted = self.__encrypt_password(password)  # private 메소드 호출
...
c1 = Circle1(5)
print('c1', c1.get_area())  # 메소드 호출: 괄호 필요
c1.set_radius(10)
print()
print('c1', c1.get_area())  # 메소드 호출: 괄호 필요
print()
