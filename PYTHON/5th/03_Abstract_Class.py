...
왜 추상 클래스를 사용할까?
1. 구현 강제: 중요한 메소드를 빼먹지 않게 강제
2. 일관성: 모든 자식이 같은 메소드를 가지게 보장
...
class Dog_Bad(Animal_Bad):
    ...
    def eat(self):
        print("강아지가 밥을 먹습니다.")
    # make_sound() 구현하는 걸 깜빡!


# 문제 발생!
dog_bad = Dog_Bad()
dog_bad.make_sound()  # 아무 것도 안 일어남 - 버그!
print("아무 소리도 안 남... 버그 발생!")
...
class Animal(ABC):  # ABC를 상속받아 추상 클래스로 만듦
    ...
    @abstractmethod  # 이 데코레이터가 추상 메소드를 표시
    def make_sound(self):
        """소리내기 - 자식이 반드시 구현해야 함!"""
...
class Dog(Animal):
    """올바른 개 클래스 - 추상 메소드 구현함"""

    def make_sound(self):  # 추상 메소드 구현 (필수!)
        print("멍멍!")
...
class 추상클래스이름(ABC):  # ABC 상속 필수!
    ...
    @abstractmethod  # 추상 메소드 표시
    def 추상메소드이름(self):
        """이 메소드는 자식 클래스에서 반드시 구현해야 함!"""
        pass  # 내용은 비워둠
...
class Circle(Shape):
    """원 - Shape의 모든 추상 메소드 구현"""

    def __init__(self, radius):
        self.radius = radius

    def area(self):  # 추상 메소드 구현 (필수!)
        return 3.14 * self.radius * self.radius

    def perimeter(self):  # 추상 메소드 구현 (필수!)
        return 2 * 3.14 * self.radius


class Rectangle(Shape):
    """직사각형 - Shape의 모든 추상 메소드 구현"""
...
# ==========================================
# 실전 예제 2: 일반 메소드 + 추상 메소드 혼합
# ==========================================
print("\n=== 실전 예제 2: 동물 클래스 (혼합형) ===")

class Animal(ABC):
    """동물 추상 클래스 - 일반 메소드와 추상 메소드 혼합"""
...
class Dog(Animal):
    ...
    def make_sound(self):  # 추상 메소드 구현
        print(f'{self.name}: 멍멍! 왈왈!')

    def move(self):  # 추상 메소드 구현
...
class Bird(Animal):
    ...
    def move(self):  # 추상 메소드 구현
        print(f'{self.name}이(가) 날개로 날아갑니다.')
