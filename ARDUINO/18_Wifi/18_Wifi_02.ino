...
bool sendCommand(String cmd, String expect, int timeout) {
    ...
    while (millis() - t < timeout) { // timeout 동안만 동작하겠다
        while (esp8266.available()) { // esp-01이 보낸 데이터가 있는지 확인
            ...
            if (buf.indexOf(expect) != -1) { // 전달받은 문자열 중 내가 원하는 단어가 있으면 true 리텅
                return true;
            }
        }
    }
...
void setup() {
    ...
    sendCommand(String("AT+CWJAP=\"") + ssid + "\",\"" + password + "\"", "GOT IP", 15000); // 위에 작성한 공유기로 접속해라
...
void sendDataToServer(int temp, int hum) {
    String url = "/data?temperature=" + String(temp) + "&humidity=" + String(hum); // 쿼리스트링으로 HTTP GET 요청 경로 작성
