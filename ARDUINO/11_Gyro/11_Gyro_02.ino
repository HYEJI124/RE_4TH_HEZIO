...
/*
    ...
    GyX, GyY, GyZ: XYZ축의 자이로스코프 값(기울어진 정도)
*/
...
void setup() {
    ...
    Wire.beginTransmission(MPU); // I2C 통신 시작 + 어떤 센서와 통신할 지 주소를 지정
    Wire.write(0x6B); // 전원 관리 레지스터 선택
    Wire.write(0); // 0x6B라는 레지스터의 값으 0으로 설정
...
void loop() {
    ...
    // 데이터 읽어오기
    // requestFrom의 매개변수: (0x68이라는 주소를 가진 I2C 통신을 하는 장치에, 14바이트를 읽어와, 다 읽었으면 통신 종료)
    Wire.requestFrom(MPU, 14, true);

    /*
        자이로/가속도 센서는 하나의 센서값을 2바이트(16비트)로 전송
        아두이노에서는 상위 바이트와 하위 바이트 총 2개를 받게 됨(아두이노는 한 번에 1바이트만 전송이 가능)
            아두이노 I2C 통신은 한 번에 1바이트(8비트)만 전송이 가능하기 때문
            .read()를 2번 작성해서 상위 바이트와 하위 바이트를 받아옴
        
        <<: 비트를 왼쪽으로 이동시키는 연산자
            x << n: x의 비트를 왼쪽으로 n만큼 이동시키고, 오른쪽 부족한 자리는 0을 채움
